...
import { AdminDeleteUser, AdminGetUsers, AdminIsAuthenticated } from '../../AdminApiCalls/AdminAuth';
...
const AdminDashBoard = () => {
  ...
  const handleDeleteData = (userId) => {
    AdminDeleteUser(adminId, userId, admintoken).then((data) => {
      if(data.msg === "User account and associated URLs deleted successfully"){
  ...
  return (
    <div style={containerStyle}>
      <div className="card mt-5 mb-5" style={cardStyle}>
        ...
        <div className="scroll-container overflow-hidden">
          ...
          {users.map(user => (
            <div className="card user-card text-white m-4" style={{ backgroundColor: "#1E1E1F" }} key={user.id}>
              <div className="card-body" style={{ backgroundColor: "#181818" }}>
                ...
                <button className="btn btn text-danger" onClick={() => handleDeleteData(user._id)} style={{ backgroundColor: "#172627" }}>Delete</button>
                  <Link to={`/admin/${user._id}/${adminId}`}>
                  <button className="btn btn text-danger" onClick={() => handleUserData()} style={{ backgroundColor: "#172627" }}>Details</button>
